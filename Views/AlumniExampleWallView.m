...
#define NEWS_PREDICATE        [NSPredicate predicateWithFormat:@"type == %d", ALUMNI_NEWS_TY]

@interface AlumniExampleWallView()
...
@implementation AlumniExampleWallView
...
- (void)loadLatestAlumniNews {
  ...
  NSString *param = [NSString stringWithFormat:@"<page>%d</page><page_size>%@</page_size><news_type>%d</news_type>", 0, ITEM_LOAD_COUNT, ALUMNI_NEWS_TY];
  
  NSString *url = [CommonUtils geneUrl:param itemType:LOAD_ALUMNI_NEWS_TY];
  
  WXWAsyncConnectorFacade *connFacade = [self setupAsyncConnectorForUrl:url
                                                            contentType:LOAD_ALUMNI_NEWS_TY];
...
connectTriggerDelegate:(id<WXWConnectionTriggerHolderDelegate>)connectTriggerDelegate
  ...
  if (self) {
    ...
    if (![WXWCoreDataUtils objectInMOC:MOC
      ...
      [self loadAlumniNewsDataWithPredicate:[NSPredicate predicateWithFormat:@"type == %d", ALUMNI_NEWS_TY]];
