...
@implementation StoreEntranceCell
...
openStoreListAction:(SEL)openStoreListAction {
  ...
  self = [super initWithStyle:style
       ...
       imageDisplayerDelegate:imageDisplayerDelegate
                          MOC:MOC
                     detailVC:detailVC
                   openAction:openStoreListAction];
  if (self) {
    ...
    _branchLabel = [[self initLabel:CGRectZero
                        ...
                        shadowColor:TRANSPARENT_COLOR] autorelease];
    _branchLabel.font = BOLD_FONT(13);
...
- (void)drawCellWithWelfare:(Welfare *)welfare height:(CGFloat)height {
  ...
  [self fetchStoreImageWithUrl:welfare.storeImageUrl];
  
  [super drawCellWithWelfare:welfare
                       title:LocaleStringForKey(NSAllowedBranchsTitle, nil)
                      height:height];
  ...
  _nameLabel.text = welfare.storeName;
  ...
  _addressLabel.text = welfare.storeAddress;
  ...
  _branchLabel.text = [NSString stringWithFormat:LocaleStringForKey(NSCheckAllStoreMsg, nil), welfare.storeCount];
