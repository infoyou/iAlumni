...
+ (void)showWithStatus:(NSString*)status {
  [[KGStatusBar sharedView] showWithStatus:status barColor:[UIColor blackColor] textColor:[UIColor whiteColor]];
}

+ (void)showErrorWithStatus:(NSString*)status {
  [[KGStatusBar sharedView] showWithStatus:status barColor:[UIColor colorWithRed:97.0/255.0 green:4.0/255.0 blue:4.0/255.0 alpha:1.0] textColor:[UIColor whiteColor]];
  [KGStatusBar performSelector:@selector(dismiss) withObject:self afterDelay:5.0 ];
}
...
- (void)showWithStatus:(NSString *)status barColor:(UIColor*)barColor textColor:(UIColor*)textColor{
  ...
  if(labelText) {
    CGSize stringSize = [labelText sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(self.topBar.frame.size.width - MARGIN * 2, self.topBar.frame.size.height)];
...
- (UIView *)topBar {
  if(!_topBar) {
    _topBar = [[UIView alloc] initWithFrame:CGRectMake(0.f, 0.f, [self rotatedSize].width, 20.0f)];
