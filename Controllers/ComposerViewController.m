...
- (void)uploadPhotoForServiceItem {
  ...
  [(ECAsyncConnectorFacade *)self.connFacade addPhotoForServiceItem:self.selectedPhoto
...
imagePickerSourceType:(UIImagePickerControllerSourceType)imagePickerSourceType {
  ...
  if (self) {
    ...
    switch (photoTakerType) {
      case SERVICE_ITEM_AVATAR_TY:
        _contentType = ADD_PHOTO_FOR_SERVICE_ITEM_TY;
        break;
...
- (void)initTextComposer {
  ...
  if (/*_contentType == SEND_POST_TY || */_contentType == SEND_EVENT_DISCUSS_TY) {
