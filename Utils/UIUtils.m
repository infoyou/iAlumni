...
+ (void)showNotificationOnTopWithMsg:(NSString *)msg
    ...
    if (showingNotification || [AppManager instance].sessionExpired) {
        return;
    }
    ...
    if (nil == infoGradientBackgroundView) {
        ...
        [((iAlumniAppDelegate*)APP_DELEGATE).window addSubview:infoGradientBackgroundView];
        
        [((iAlumniAppDelegate*)APP_DELEGATE).window bringSubviewToFront:infoGradientBackgroundView];
    }
...
+ (void)addInfoLabelToWindow:(MessageType)msgType
    ...
    if (nil == infoBackgroundView) {
        ...
        infoBackgroundView.center = ((iAlumniAppDelegate*)APP_DELEGATE).window.center;
    ...
    [((iAlumniAppDelegate*)APP_DELEGATE).window addSubview:infoBackgroundView];
    
    [((iAlumniAppDelegate*)APP_DELEGATE).window bringSubviewToFront:infoBackgroundView];
}
...
+ (void)showNotificationWithMsg:(NSString *)msg msgType:(MessageType)msgType {
    ...
    if ([AppManager instance].sessionExpired) {
        return;
    }
    
    if (nil == infoLabel) {
        if (![NULL_PARAM_VALUE isEqualToString:[AppManager instance].errDesc] && [AppManager instance].errDesc.length > 0) {
...
+ (void)closeAsyncLoadingView {
    ...
    if (asyncLoadingBackgroundView && asyncLoadingLabel/* && operaFacebookImageView*/) {
        ...
        asyncLoadingBackgroundView.frame = CGRectMake(0, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.height, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT);
        [UIView commitAnimations];
    }
}
...
+ (void)reverseFacebook {
    ...
    operaFacebookImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"beijingOpera%d", index]];
...
+ (void)showAsyncLoadingView:(NSString *)message toBeBlockedView:(UIView *)toBeBlockedView {
    ...
    if (nil == asyncLoadingBackgroundView) {
        asyncLoadingBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.height, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT)];
        asyncLoadingBackgroundView.backgroundColor = [UIColor blackColor];
        asyncLoadingBackgroundView.alpha = 0.7f;
        [((iAlumniAppDelegate*)APP_DELEGATE).window addSubview:asyncLoadingBackgroundView];
    }
    ...
    CGSize size = [asyncLoadingLabel.text sizeWithFont:asyncLoadingLabel.font
                                     constrainedToSize:CGSizeMake(200, CGFLOAT_MAX)
                                         lineBreakMode:NSLineBreakByWordWrapping];
    asyncLoadingLabel.frame = CGRectMake(((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.width - MARGIN * 2 - size.width, ASYNC_LOADING_VIEW_HEIGHT/2 - size.height/2, size.width, size.height);
    ...
    asyncLoadingBackgroundView.frame = CGRectMake(0, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.height - ASYNC_LOADING_VIEW_HEIGHT, ((iAlumniAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT);
...
+ (BOOL)shouldLoadLatestChat:(UIScrollView *)scrollView
    ...
    if (scrollView.isDragging) {
        ...
        if ((*currentStatus == PULL_FOOTER_NORMAL) &&
            ...
            *currentStatus = PULL_FOOTER_LOADING;
...
+ (BOOL)shouldLoadOlderChat:(UIScrollView *)scrollView
    ...
    if (scrollView.isDragging) {
        ...
        if (*currentStatus == PULL_HEADER_PULLING && scrollView.contentOffset.y > -(HEADER_TRIGGER_OFFSET) && scrollView.contentOffset.y < 0.0f && !reloading) {
            *currentStatus = PULL_HEADER_NORMAL;
            
            return NO;
        } else if (*currentStatus == PULL_HEADER_NORMAL && scrollView.contentOffset.y < -(HEADER_TRIGGER_OFFSET) && !reloading) {
            *currentStatus = PULL_HEADER_PULLING;
            return YES;
        }
    }
...
+ (UIView *)getKeyboard {
    ...
    for (UIWindow *window in [windows reverseObjectEnumerator]) {
        for (UIView *view in window.subviews) {
            ...
            if ([@"UIKeyboard" isEqualToString:className]) {
                return view;
            }
        }
    }
...
+ (void)drawActivityView:(NSString *)text {
    ...
    if (nil == activityBackgroundView) {
        ...
        activityBackgroundView.center = ((iAlumniAppDelegate*)APP_DELEGATE).window.center;
    }
    ...
    if (text && [text length] > 0) {
        ...
        if (nil == loadingLabel) {
            ...
            loadingLabel.center = ((iAlumniAppDelegate*)APP_DELEGATE).window.center;
    ...
    [((iAlumniAppDelegate*)APP_DELEGATE).window addSubview:activityBackgroundView];
    [((iAlumniAppDelegate*)APP_DELEGATE).window bringSubviewToFront:activityBackgroundView];
