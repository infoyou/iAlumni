...
- (id)initWithDelegate:(id<ECConnectorDelegate>)delegate
...
- (void)asyncTimeoutHandler {
  if (_running) {
    if (_showAlertMsg) {
      [UIUtils showNotificationOnTopWithMsg:LocaleStringForKey(NSTimeoutMsg, nil)
                                    msgType:ERROR_TY
...
- (void)handleNoResponseStatus:(BOOL)showAlertMsg {
  ...
  /*
   if (showAlertMsg) {
   [UIUtils alert:LocaleStringForKey(NSNoResponseMsg, nil)
   message:nil];
   }
   */
  if (_getMethod) {
...
- (NSData *)syncGet:(NSString *)urlStr {
    ...
    [UIUtils showNotificationOnTopWithMsg:LocaleStringForKey(NSConnectionErrorMsg, nil)
                                  msgType:ERROR_TY
                       belowNavigationBar:YES];
    return nil;
  }
...
- (NSData *)syncPost:(NSString *)aUrl paramDic:(NSDictionary *)paramDic {
  ...
  self.postParam = [self.postParam stringByReplacingOccurrencesOfString:@"\n" withString:@"^||^"];
	...
	NSString *contentType = [NSString stringWithFormat:@"multipart/form-data; boundary=%@", IALUMNI_FORM_BOUNDARY];
